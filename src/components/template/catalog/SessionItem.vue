<template>
  ...
  >
    <div class="flex items-center gap-5 max-[1030px]:flex-col">
      ...
      <p>{{ formatPrice(item.price) + 'сум' }}</p>
    </div>
    <p>{{ formatTime(item.time) }}</p>
    <div class="flex gap-[10px] items-center justify-center max-[1030px]:mt-5  max-[800px]:flex-col">
      <button
          @click="openEditModal"
...
export default {
  ...
  mounted() {
    const originalTime = new Date(this.item.time);
    const end = new Date(originalTime.getTime() + this.item.movie_duration * 60000).toISOString();

    let today  = new Date().toISOString();
    if (today > end){
      this.last = true
    } else if (today < this.item.time){
      this.future = true
    } else if (today >= this.item.time && today <= end){
      this.now = true
    }
  },
}
